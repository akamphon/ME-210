...
\tikzset{>=latex}
\usetikzlibrary{arrows,calc,decorations,decorations.pathreplacing,shapes,decorations.pathmorphing,patterns,snakes,decorations.fractals,shadings,lindenmayersystems,shadows}
\usepackage{pgfplots}
...
\begin{equation}
  ...
  \begin{array}{c}
    \sigma_x(\Delta y\Delta z) + \sigma_x'(\Delta y\Delta z) = 0\\
    \sigma_x =  - \sigma_x'
  \end{array}
\end{equation}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[>=latex]
    % axes
    \draw [<->] (0,8) node[left]{$\sigma$} --++ (-90:8) node(O){} --++ (0:11) node[below]{$\varepsilon$};
    % braces
...
\begin{example} Statically indeterminate torsional analysis in a cylindrical shaft
  ...
  \begin{tikzpicture}
    ...
    \draw [|<->|] (cyl.west) ++ (-90:1) --++ (0:5.5) node[midway, fill=white]{0.5 m};
    \draw [|<->|] (cyl.west) ++ (-90:1) ++ (0:5.5) --++ (0:4.5) node[midway, fill=white]{0.4 m};
  \end{tikzpicture}
\end{figure}
...
\begin{exercises}
  ...
  \item You are chewing a bubblegum that has a volume of 2 cm$^3$ and
  ...
  \item A 500-N highway sign is attached to the end of an L-shaped,
...
\item An L-shaped bar is fixed on one end at the wall. The bar has a constant circular cross section with the radius of 1 cm. The bar is made of a \emph{ductile} steel whose Young's modulus $E$ = 210 GPa, yield strength $S_{yield}$ = 300 MPa, and ultimate tensile strength $S_{ut}$ = 450 MPa.
  ...
  \item (5 points) Draw a Mohr's circle representing the state of stress (prinical and maximum shear stresses) at the critical point.
...
\begin{exercises}
  ...
  \item A 3-m long springboard at a water park is used repeatedly
